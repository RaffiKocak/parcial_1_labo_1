...
int PedirCadenaProlijaConReintentos(char mensaje[], char cadena[], int tam, int intentos) {
	...
	while (intentos > 0 && (GetString(cadenaLocal, sizeof(cadenaLocal)) == -1)) {
		printf("ERROR! %s", mensaje);
		intentos--;
	}

	if (intentos > 0) {
		EmprolijarCadena(cadenaLocal);
...
void PedirCadena(char mensaje[], char cadena[], int tam, int intentos) {
	...
	while (GetString(cadenaLocal, sizeof(cadenaLocal)) == -1) {
		printf("ERROR! %s", mensaje);
	}
...
int PedirCUITConReintentos (char mensaje[], char cadenaIngresada[], int tamCadenaIngresada, int intentos) {
	...
	while (intentos > 0 && (GetString(cadenaAVerificar, sizeof(cadenaAVerificar)) == -1 || strlen(cadenaAVerificar) != 11 ||
			!VerificarFormatoCUIT(cadenaAVerificar))) {
			printf("ERROR! %s", mensaje);
			intentos--;
	}

	if (intentos > 0) {
		strncpy(cadenaIngresada, cadenaAVerificar, tamCadenaIngresada);
	} else {
		printf("\nDemasiados intentos fallidos. Ingreso de dato cancelado.\n");
		retorno = 0;
	}
...
int VerificarRangoCUIT (char numeroAVerificar[], double *numeroADevolver) {
	...
	if (numeroTransformado >= 20000000000 && numeroTransformado <= 35000000000) {
		esCUIT = 1;
		*numeroADevolver = numeroTransformado;
	}
	return esCUIT;
